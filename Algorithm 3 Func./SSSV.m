function [ff,curerror,mu,Ared, ...
                mulist,dimU,dimV, pars] = SSSV(A,theta,thetap,bounds,options)
...
while ((curerror > tol)&&(iter<max_iter)) 
    ...
    parfor ii=1:Ntrain
       [app_err_SV(ii),~] = sigma_error_all(mu_t(:,ii),pars);
    end
    [curerror,ind]=max(app_err_SV); mu=mu_t(:,ind);
    %% Delate all parameters that verifies the exit condition
    ind_2=find(app_err_SV<tol); 
    mu_t(:,ind_2)=[]; Ntrain=Ntrain-numel(ind_2); app_err_SV = zeros(Ntrain,1);
...
% Discreet grid
mu_1d = cell(p,1); Ntrain = options.Nt2; flag_log = 0;
for i=1:p
    mu_1d{i}=zeros(p,Ntrain);
